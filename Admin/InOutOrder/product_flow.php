...
try {
    ...
    $stmt_in = $conn->query("SELECT COUNT(*) AS total_in FROM Masuk_Product");
    $total_in = $stmt_in->fetch(PDO::FETCH_ASSOC)['total_in'];

    $stmt_out = $conn->query("SELECT COUNT(*) AS total_out FROM Keluar_Product");
    $total_out = $stmt_out->fetch(PDO::FETCH_ASSOC)['total_out'];

    // Fetch product data for dropdown and table
    $stmt_products = $conn->query("SELECT id_product, merek, tipe FROM Products"); // Change 'type' to 'tipe' as per your schema
    $products = $stmt_products->fetchAll(PDO::FETCH_ASSOC);

    // Combine incoming and outgoing data for the table
    $stmt_flow = $conn->query("
        SELECT 'In' AS type, id_masuk AS id, id_product, tanggal_masuk AS date, kuantitas AS quantity 
        FROM Masuk_Product
        UNION ALL
        SELECT 'Out' AS type, id_keluar AS id, id_product, tanggal_keluar AS date, kuantitas AS quantity 
        FROM Keluar_Product
        ORDER BY date DESC
    ");
                            ...
                            <select name="id_product" id="id_product" class="form-select">
                                <?php foreach ($products as $product): ?>
                                    <option value="<?= $product['id_product'] ?>">
                                        <?= htmlspecialchars($product['merek'] . ' - ' . $product['tipe']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <?php foreach ($product_flow as $flow): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    echo htmlspecialchars($product_name['merek'] . ' - ' . $product_name['tipe']);
                                    ?>
                                </td>
