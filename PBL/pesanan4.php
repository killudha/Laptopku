...
try {
    ...
    if (!$user) {
        die("User not found."); // Anda mungkin ingin menangani ini dengan lebih baik
        // exit(); // die() sudah menghentikan skrip
    }
...
// Mengambil data pesanan seperti kode asli Anda
$id_user = $_SESSION['id_user'];
$result = []; // Inisialisasi untuk menghindari error jika query gagal
try {
    $query = "
    ...
    // Tangani error pengambilan data pesanan, $result akan tetap kosong
    error_log("Error fetching orders for pesanan.php: " . $e->getMessage());
}
...
<head>
    ...
    <style>
        /* CSS untuk memindahkan judul "Pesanan" ke kiri */
        .main-content .title {
            text-align: left; /* Mengatur teks rata kiri */
            /* Anda mungkin perlu menyesuaikan margin jika diperlukan */
            /* margin-left: 0; */ /* Contoh jika Anda ingin menghilangkan margin kiri default */
        }
        .no-orders-message { /* Style untuk pesan "tidak ada pesanan" */
            text-align: center;
            padding: 20px;
            margin-top: 20px;
...
<body>
    ...
    <section class="hero">
        ...
        <div class="main-content">
            <div class="title">Pesanan</div> <!-- Judul ini akan rata kiri karena CSS di atas -->
            <div class="track">
                <p>Order Tracker</p>
                <p><?= count($result) ?> Items</p> <!-- Karakter 'x' sudah dihilangkan -->
            </div>
            ...
            <div class="status-container">
                ...
                <div class="status-item" onclick="window.location.href='pesanan4.php';" style="cursor: pointer;">
                    ...
                    <p><span class="pay">Beri<br>Penilaian</span></p>
                </div>
            </div>
            ...
            <?php else: ?>
                <div class="track"> <!-- Judul sub-bagian ini dikembalikan seperti semula -->
                    <p>Beri Penilaian</p>
                </div>
                <?php foreach ($result as $row): ?>
                    <div class="order-card">
                        ...
                        <div class="btn-payment">
                            ...
                            <?php if ($row['status_delivery'] === 'completed'): ?>
                                <button class="payment-button" onclick="window.location.href='ulasan.php?id_order=<?= $row['id_order'] ?>';">Beri Ulasan</button>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
